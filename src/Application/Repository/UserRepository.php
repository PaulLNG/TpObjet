...
use Application\Collection\UserMeetingCollection;
...
final class UserRepository
{
    ...
    public function getAllOrganiserMeeting(): array
    {
        {
            
            $query = $this->pdo->query('SELECT u.name as user_name, m.id as user_id, m.title as meeting_title, description
...
inner join communities c on c.id = m.community_id;');
    ...
    public function getMeetingsByUser(string $userId): array
    {
        $statement = $this->pdo->prepare('SELECT u.name as user_name, m.id as user_id, m.title as meeting_title, description
        ...
        FROM users u
        inner join userMeeting a on u.id = a.user_id
        ...
        while ($data = $statement->fetch()) {
            ...
            $user = new User($data['user_name']);
            $meetings[] = [
                new Meeting($data['meeting_title'], $data['description'], new \DateTimeImmutable($data['date_end']), new \DateTimeImmutable($data['date_start'])),
                new Community($data['community_id'])
            ];
        }
        return [
            $user,
    ...
    public function getUsersByMeeting(string $meetingId): UserCollection
    {
        $statement = $this->pdo->prepare('SELECT u.id as id_user , u.name as user_name
                        FROM  users u 
                        JOIN userMeeting a
                        ON a.user_id = u.id
                        JOIN meetings m
